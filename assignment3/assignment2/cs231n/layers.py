...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    x_hat = (x - mu_b.T) / np.sqrt(var_b.T + eps)
...
def batchnorm_backward(dout, cache):
  ...
  dvar = np.sum(dx_hat * (cache['x']-cache['mean']) * (-0.5) * (cache['var'] \
                                            + cache['eps'])**(-1.5), axis=0)
  dmean = np.sum(dx_hat * (-1/np.sqrt(cache['var'] + cache['eps'])) , axis=0) \
                    + dvar * ((np.sum(-2*(cache['x']-cache['mean']))) / m)
...
def batchnorm_backward_alt(dout, cache):
  ...
  dbeta = np.sum(dout, axis=0)
  dgamma = np.sum((cache['x'] - cache['mean']) * (cache['var'] +
                                                  cache['eps'])**(-1. / 2.)
                                                * dout, axis=0)
  dx = (1. / N) * cache['gamma'] * (cache['var'] \
                                    + cache['eps'])**(-1. / 2.)\
                                * (N * dout - np.sum(dout, axis=0)\
                                - (cache['x'] - cache['mean']) *(cache['var']+\
                                 cache['eps'])**(-1.0) *\
                                np.sum(dout * (cache['x']\
                                 - cache['mean']), axis=0))\
  #############################################################################
    ...
    for depth in xrange(F): #depth
      for r in xrange(0,H,S): #row
        for c in xrange(0,W,S): #column
          out[n,depth,r/S,c/S] = np.sum(x_pad[n,:,r:r+HH,c:c+WW] * w[depth,:,:,:]) \
                                    + b[depth]
